...
def apply_primitive(procedure, args, env):
    ...
    al = []
    while args != nil:
        al.append(args.first)
        args = args.second
    if procedure.use_env:
        al.append(env)
    try:
        return procedure.fn(*al)
    except TypeError:
        raise SchemeError("Incorrect argument number")
    # END PROBLEM 3
...
def do_lambda_form(vals, env):
    ...
    if vals.second == nil:
        ...
        vals = Pair("begin", vals)
    return LambdaProcedure(formals, vals, env)
    # END PROBLEM 8
...
def do_define_form(vals, env):
    ...
    if scheme_symbolp(target):
        check_form(vals, 2, 2)
        # BEGIN PROBLEM 5
        env.bindings[target] = vals[1]
        return target
        # END PROBLEM 5
    elif isinstance(target, Pair):
        # BEGIN PROBLEM 9
        symbol, formals = target.first, target.second
        env.bindings[symbol] = do_lambda_form(Pair(formals, vals.second), env)
        return symbol
        # END PROBLEM 9
    else:
