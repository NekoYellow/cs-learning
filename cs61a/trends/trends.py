...
def extract_words(text):
    ...
    """
    "*** YOUR CODE HERE ***"
    char_array = [c if c in ascii_letters else ' ' for c in text]   # c in ascii_letters <=> c.isalpha()
...
def make_sentiment(value):
    ...
    "*** YOUR CODE HERE ***"
    return {'has': value!=None, 'value': value}
...
def analyze_tweet_sentiment(tweet):
    ...
    for word in words:
        ...
        if has_sentiment(senti):
            total += sentiment_value(senti)
            cnt += 1
    avg = None if cnt == 0 else total / cnt
...
def find_centroid(polygon):
    ...
    for i in range(N):
        ...
        atri = (x1*y2 - x2*y1)/2.0
        xc += atri* (.0+x1+x2)/3.0
        yc += atri* (.0+y1+y2)/3.0
        area += atri
    if area == .0:
        return x2, y2, .0
    return xc/area, yc/area, abs(area)
...
def find_state_center(polygons):
    ...
    for poly in polygons:
        x, y, a = find_centroid(poly)
        lat += x*a
        lon += y*a
        area += a
    return make_position(lat/area, lon/area)
...
def group_tweets_by_state(tweets):
    ...
    for tweet in tweets:
        nearest, min_dist = '', float('inf')
        pos = tweet_location(tweet)
        for state in centers:
            dist = geo_distance(pos, centers[state])
            if dist < min_dist:
                nearest, min_dist = state, dist
        tweets_by_state[nearest] = tweets_by_state.get(nearest, []) + [tweet]
...
def average_sentiments(tweets_by_state):
    ...
    for state, tweets in tweets_by_state.items():
        total_senti, cnt = None, 0
        for tweet in tweets:
            senti = analyze_tweet_sentiment(tweet)
            if has_sentiment(senti):
                if total_senti == None:
                    total_senti = sentiment_value(senti)
                else:
                    total_senti += sentiment_value(senti)
                cnt += 1
        if total_senti != None:
            averaged_state_sentiments[state] = total_senti / cnt
